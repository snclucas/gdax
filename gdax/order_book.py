...
class OrderBook(WebsocketClient):
    def __init__(self, product_id='ETH-USD', log_to=None):
    ...
    def get_nums(self):
        nums = {"buys": self.num_buys, "sells": self.num_sells,
                "buy_size": self.size_buys, "sell_size": self.size_sells,
                "bid": self.get_bid(), "ask": self.get_ask()}
        self.num_buys = 0
        self.num_sells = 0
        self.size_sells = 0
        self.size_buys = 0
    ...
    def match(self, order):
        if order['side'] == 'buy':
            ...
            self.size_buys = self.num_buys + float(order['size'])
        if order['side'] == 'sell':
            self.num_sells = self.num_sells + 1
            self.size_sells = self.num_sells + float(order['size'])
    ...
    def get_num_bids(self, price):
        return len(self.get_bids(price))
